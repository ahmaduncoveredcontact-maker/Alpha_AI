...
import { Pencil, ExternalLink, Trash2, Users, CheckCircle, Clock } from 'lucide-react';
...
export default function AdminDashboardClient({ clients }: { clients: Client[] }) {
  const [search, setSearch] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');

  const filteredClients = clients.filter(client => {
    const matchesSearch = client.business_name.toLowerCase().includes(search.toLowerCase());
    const status = client.vapi_assistant_id ? 'active' : 'pending';
    const matchesStatus = statusFilter === 'all' || status === statusFilter;
    return matchesSearch && matchesStatus;
  });

  const total = clients.length;
  const active = clients.filter(c => c.vapi_assistant_id).length;
  const pending = total - active;

  const handleDelete = async (slug: string, name: string) => {
    if (!confirm(`⚠️ Are you sure you want to delete "${name}"? This action cannot be undone.`)) return;
    if (!confirm(`Final confirmation: Delete "${name}" and all associated data?`)) return;
    try {
                  ...
                  filteredClients.map((client) => (
                    <tr key={client.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                        ...
                        <button
                          onClick={() => handleDelete(client.slug, client.business_name)}
